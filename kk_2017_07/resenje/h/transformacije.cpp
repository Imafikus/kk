...
extern Function *Atan, *Sqrt, *Cos, *Sin;
Value* Rotation::codegen() const {
  ...
  // novi ugao
  Value *Phi = Builder.CreateFAdd(ConstantFP::get(TheContext, APFloat(_num)),
				  Builder.CreateCall(Atan,
						     Builder.CreateFDiv(Second,
									First,
									"divtmp"),
						     "arkustangens"),
				  "phi");
  // moduo
  Value *Moduo = Builder.CreateCall(Sqrt,
				    Builder.CreateFAdd(Builder.CreateFMul(First, First, "x^2"),
						       Builder.CreateFMul(Second, Second, "y^2"),
						       "x^2y^2"),
				    "moduo");
  ...
  Idxs11.push_back(ConstantInt::get(TheContext, APInt(32, 0)));
  Value* GEP1 = Builder.CreateGEP(VektorTip, Alloca, Idxs11, "x_coord");
  Builder.CreateStore(Builder.CreateFMul(Builder.CreateCall(Cos, Phi, "cosinus"), Moduo, "x"), GEP1);
  ...
  Idxs22.push_back(ConstantInt::get(TheContext, APInt(32, 1)));
  Value* GEP2 = Builder.CreateGEP(VektorTip, Alloca, Idxs22, "y_coord");
  Builder.CreateStore(Builder.CreateFMul(Builder.CreateCall(Sin, Phi, "sinus"), Moduo, "y"), GEP2);
...
Value* TransformationId::codegen() const {
  Function *CalleeF = Functions[_name];
  if (!CalleeF) {
    cerr << "Transformacija " << _name << " nije definisana" << endl;
    return NULL;
  }
...
Value* Composition::codegen() const {
  ...
  // za svako kompoziciju pravimo obavezno po jednu fju
  Function *TheFunction = Function::Create(FT, Function::ExternalLinkage, "composition", TheModule);
  ...
  // ucitamo vektor koji je argument fje
  Value* RetVal = Builder.CreateLoad(VektorTip, NamedValues["ARG"], "arg");
  // nad njim pozivamo desnu transformaciju
  Function* r = (Function*)_r->codegen();
  RetVal = Builder.CreateCall(r, RetVal, "calltmp");
  // pa onda levu
  Function* l = (Function*)_l->codegen();
  RetVal = Builder.CreateCall(l, RetVal, "calltmp");
